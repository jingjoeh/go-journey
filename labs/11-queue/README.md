...
Process a job with a bounded retry policy that respects cancellation.

## Requirements

- Return on first success.
- Attempt no more than maxAttempts.
- Stop immediately when context is cancelled.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func Process(ctx context.Context, maxAttempts int, handle func(context.Context) error) error`.
