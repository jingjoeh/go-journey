...
Record bounded request outcomes without using high-cardinality labels.

## Requirements

- Count by route template and status class.
- Be safe for concurrent calls.
- Return a snapshot that callers cannot mutate internally.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func (m *Metrics) Record(route string, status int); func (m *Metrics) Snapshot() map[string]uint64`.
