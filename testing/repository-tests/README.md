...
Senior backend engineers need to reason about database contracts, constraints, transactions, query behavior, and cleanup. The skill matters because local correctness is not enough: choices must remain understandable under concurrency, failure, growth, and operational pressure.
...
1. Explain repository tests in your own words, including one guarantee and one non-guarantee.
