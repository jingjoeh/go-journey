...
Senior backend engineers need to reason about values, types, methods, packages, modules, and idiomatic API design. The skill matters because local correctness is not enough: choices must remain understandable under concurrency, failure, growth, and operational pressure.
...
1. Explain go language in your own words, including one guarantee and one non-guarantee.
