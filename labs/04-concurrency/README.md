# Concurrency: Bounded Parallel Map

> Work in `starter/`. Do not open `solution/` until you have a tested attempt.

## Objective

Run work concurrently while preserving order, bounding concurrency, and honoring cancellation.

## Requirements

- Square every input and preserve order.
- Never exceed workers active calls.
- Reject workers below one and stop on cancellation.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func SquareAll(ctx context.Context, values []int, workers int) ([]int, error)`.
