# Production Readiness: Graceful Runner

> Work in `starter/`. Do not open `solution/` until you have a tested attempt.

## Objective

Coordinate startup, cancellation, graceful shutdown, and error propagation.

## Requirements

- Run the server until cancellation or failure.
- Call Shutdown with a bounded context.
- Treat ErrServerClosed as a normal stop.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func Run(ctx context.Context, server Server, shutdownTimeout time.Duration) error`.
