...
Senior backend engineers need to reason about HTTP contracts, authentication, error bodies, compatibility, and end-to-end behavior. The skill matters because local correctness is not enough: choices must remain understandable under concurrency, failure, growth, and operational pressure.
...
1. Explain api tests in your own words, including one guarantee and one non-guarantee.
