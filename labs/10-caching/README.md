# Caching: Stampede-Safe Loader

> Work in `starter/`. Do not open `solution/` until you have a tested attempt.

## Objective

Implement a concurrency-safe cache that coalesces simultaneous loads.

## Requirements

- Return cached values without reloading.
- Only one load for concurrent misses of a key.
- Do not cache loader errors.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func (c *Cache) Get(ctx context.Context, key string, load func(context.Context) (string, error)) (string, error)`.
