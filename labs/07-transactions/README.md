...
Place a money transfer inside one transaction and preserve the balance invariant.

## Requirements

- Reject non-positive amounts.
- Debit then credit in one transaction callback.
- Return errors without hiding their cause.

## Constraints

- Use the Go standard library.
- Keep the public contract focused on `func Transfer(ctx context.Context, tx Transactor, from, to string, amount int64) error`.
