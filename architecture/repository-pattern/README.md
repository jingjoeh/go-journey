...
Senior backend engineers need to reason about aggregate persistence, query needs, interface placement, and abstraction leaks. The skill matters because local correctness is not enough: choices must remain understandable under concurrency, failure, growth, and operational pressure.
...
1. Explain repository pattern in your own words, including one guarantee and one non-guarantee.
